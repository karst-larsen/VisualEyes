...
function VisualEye({darkMode, steps, playing, timeNode, openHatArray})  {
    ...
    return (
        <div className={`visual-eye ${testContainer[timeNode]?.id === timeNode ? 'visual-eye--active' : 'visual-eye--inactive'}`}>
            ...
            <img src={darkMode ? visualEyeInvert : visualEye} alt="visual-eye" className={`visual-eye__eye ${testContainer[timeNode]?.id === timeNode ? 'visual-eye--active' : 'visual-eye--inactive'}`} />
            <img src={transparentEye} alt="visual-eye" className={`visual-eye__transparent-eye ${playing ? 'visual-eye__transparent-eye--active' : ''}`} />
            <img src={darkMode ? transparentEyeInvert : transparentEye} alt="visual-eye" className={`visual-eye__transparent-eye-pulse ${testContainer[timeNode]?.id === timeNode ? 'visual-eye__transparent-eye-pulse--active' : 'visual-eye__transparent-eye-pulse--inactive'}`} />
        </div>
    );
}
